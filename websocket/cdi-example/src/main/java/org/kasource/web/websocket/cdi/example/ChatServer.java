...
import org.kasource.web.websocket.event.extension.BroadcastTextWebSocketMessageEvent;
...
public class ChatServer {
    ...
    @Inject @Any 
    private Event<BroadcastTextWebSocketMessageEvent> broadcastEvent;
    ...
    public void onClientConnect(@Observes @Chat WebSocketClientConnectionEvent event) throws NoSuchWebSocketClient, IOException {
        sendEvent.fire(new SendWebSocketTextMessageEvent(event, "Welcome " + event.getClientId()));
        broadcastEvent.fire(new BroadcastTextWebSocketMessageEvent(event, event.getClientId() + " left the conversation."));
    }
    ...
    public void onMessage(@Observes @Chat WebSocketTextMessageEvent event) {
        broadcastEvent.fire(new BroadcastTextWebSocketMessageEvent(event, event.getClientId() + " says: " + event.getMessage()));
    }
